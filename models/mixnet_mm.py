...
def mixnet_mm_pretrain(pretrained=False, **kwargs):

    model = MixNet(net_type='mixnet_m',beat_net_type='mixnet_m',num_classes=55)
    if pretrained:
        print("mixnet_mm pretrain")
        model.load_state_dict(torch.load("./round2/mixnet_mm_not_all_data_transform_best_weight.pth", map_location='cpu')['state_dict'])
    return model

def mixnet_mm(pretrained=True, **kwargs):

    model = MixNet(net_type='mixnet_m',beat_net_type='mixnet_m',num_classes=55)
    if pretrained:
        print("mixnet_mm pretrain")#"./round2/mixnet_mm_not_all_data_transform_best_weight.pth"
        model.load_state_dict(torch.load(config.round1_pretrain_mixnet_mm_weight, map_location='cpu')['state_dict'])
    return model


def mixnet_mm_predict(pretrained=False, **kwargs):

    model = MixNet(net_type='mixnet_m',beat_net_type='mixnet_m',num_classes=34)
    if pretrained:
        print("mixnet_mm pretrain")
        model.load_state_dict(torch.load("./round2/mixnet_mm_not_all_data_transform_best_weight.pth", map_location='cpu')['state_dict'])
    return model
